...
class Signature {
  constructor(apiKey, apiSecret) {
    if (this.apikey || this.apiSecret) {
      throw new Error(
        "apikey and apiSecret must be suppiled to calculate signature"
      );
    }
    this.apikey = apiKey;
  ...
  calculate(nonce, timestamp) {
    ...
    const signatureEncoded = encodeURIComponent(
      crypto.enc.Base64.stringify(signatureSigned)
    );
...
class Result {
  constructor(key, timestamp, nonce, encodedSignature) {
    ...
    this.authorization = `Signature keyId="${key}",algorithm="hmac-sha1",headers="date x-mod-nonce",signature="${encodedSignature}"`;
